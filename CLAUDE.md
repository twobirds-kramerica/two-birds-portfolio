...
- SESSION LENGTH RULE (S-MCP-RELIABILITY-001, 2026-04-22): A single Claude Code session should not exceed ~3 hours of continuous active work or ~40 sequential tool calls against the same Notion data source without a deliberate break. Longer sessions compound two failure modes: (1) cache-warmth effects that make silent MCP failures harder to detect, (2) decision fatigue that produces the kind of premature "drained" calls made repeatedly in the 2026-04-22 session. When approaching the limit: finalise current sprint cleanly, commit + push + update SESSION-STATE, then stop. Resume in a fresh session.
- MCP WRITE SAFETY RULE (S-MCP-RELIABILITY-001, 2026-04-22): Every Notion write done programmatically from Claude Code should go through `hal-stack/mcp-reliability/notion-write-safe.py`'s `safe_notion_write()` wrapper. Adds retry-with-backoff + response verification + optional server-side read-back. Logs every attempt to `logs/mcp-write-log.txt`; exhausted writes fall back to `logs/mcp-write-fallback.json` for manual recovery. Prevents the S-041 class of bug (POST succeeded, client crashed, work looked failed). `hal-stack/mcp-reliability/heartbeat-check.py` provides periodic liveness probes; `session-state-verify.py` is a pre-sprint preflight.
- REPO VISIBILITY RULE (Security Cleanup Sprint, 2026-04-23/24): Any repo containing real user data (names, addresses, financials, personal notes) must be private by default. Public is opt-in only and requires explicit confirmation. Origin: `kevins-apartment-search` was created public despite being a private tracker for Kevin Burnett's actual apartment shortlist; the live site was correctly noindex but the GitHub source was discoverable. When flipping a public repo to private on the current org plan, GitHub Pages goes 404 (Pages-on-private requires Pro tier or higher). Mitigations: (a) keep public + scrub identifying details, (b) flip private + accept Pages downtime + serve via alternative host, (c) upgrade plan. Whichever route, the repo's data classification must be decided BEFORE first commit, not retro-fitted after a security audit.
...
"just go" — single-sprint autonomous authorization in NORMAL mode (no max-mode chain, no persistent posture change). Authorises Claude to: (1) pick the highest-LOE executable scope from any available source (Notion Backlog item with scope, research proposal in `hal-stack/research/*.md`, sprint-queue.md READY item, or in-session candidate Claude has already scoped), (2) flip Backlog→Ready on Notion if the source is a Notion item (or file a new Backlog item via `create_backlog_item` if source is local-only), (3) execute ONE sprint end-to-end, (4) commit per phase, (5) update SESSION-STATE.md, (6) push, (7) STOP and return control to Aaron for next decision. Does NOT chain to the next sprint. Does NOT persist beyond the single sprint. Fixes RI-006 — the normal-mode empty-queue "next sprint" loop. Before picking the source item, Claude MUST git-log-grep the sprint ID / keywords to ensure the candidate is not already shipped (per feedback_git_log_grep_proposals memory).
