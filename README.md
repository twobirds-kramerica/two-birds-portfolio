...
Four files, about two minutes. You will know: what Two Birds is, what shipped last, what's ready to ship, and what's blocked on human input.
...
- **Template:** Two Birds Project Template. Every new repo clones from this; it ships an a11y baseline, axe-core CI, AUDIT.md structure, self-hosted fonts, and more.
...
- **Standing rules** in CLAUDE.md are non-negotiable (static-only, Canadian English, commit after every phase, and so on).
...
Local-first, sovereign-by-design development and automation infrastructure. See **[hal-stack/README.md](hal-stack/README.md)** for the full spec: sovereignty model (L1-L4), machine roster (Pentium Silver, ThinkPad i5, EZbook), persona system (Scrappy Pack + Founding Board), and architectural details.

The **Sovereignty Float Model** (L1-L4): every component must be designed so dropping between layers is a configuration change, not a rebuild. L1 is commercial cloud (default); L4 is air-gapped local.
...
| `RELIABILITY-ISSUES.md` | Log of systemic failures and their fixes (RI-001 through RI-007 at time of writing) |
...
5. End every work unit with a SESSION-STATE update and `git push`.
...
The **Founding Board** (22 personas across 6 departments; see `hal-stack/personas/`) only runs when the sprint queue is empty. Legal (Helen) and CTO/Engineering (Naveen) hold veto cards.
