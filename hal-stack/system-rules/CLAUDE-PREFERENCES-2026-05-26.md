...
3. If Notion fails, fall back to: https://raw.githubusercontent.com/twobirds-kramerica/two-birds-portfolio/master/SESSION-STATE,�d
4. If both fail, ask you to paste SETION-STATE.md contents
...
➕ NEW — description of new feature not previously asked (10-30% credits)
...
Tag claims with [HYPOTHESISU,M[UNKNOWN], [UNVERIFIED CLAIM], or [FACT].
...
# Presentation tool rule — LLM-agnostic (May 26, 2026)

Applies to: Gamma, Canva, any slide or visual generation tool. Applies regardless of which LLM is calling the tool.

## Data integrity rule (permanent)

When calling any presentation or visual generation tool, the prompt MUST include this instruction verbatim:

"You are acting as a layout and design engine only. Do not infer, assume, extrapolate, or add any data, statistics, dates, claims, or statements not explicitly present in the source content provided. If content is missing from a field or section, leave a visible placeholder. Do not generate content to fill gaps. Do not interpret or editorialize."

This rule applies to every tool call, every session, every LLM. It is not optional and is not context-dependent.

## Workflow rule (permanent)

Gamma and similar tools create a new file on every generate_from_template or generation call — they do not edit in place. The correct workflow is:

1. Claude holds and validates all data before any tool call.
2. The generation tool receives only complete, verified content —"never partial or ambiguous inputs.
3. Every generation produces a new URL. The old URL is superseded. Update any links that referenced the old URL before closing the task.
3. Never attempt in-place edits on Gamma outputs. Always generate fresh from source.

## Access note

Claude cannot be added as a named team member in Gamma or similar tools. Access is managed through the connected MCP session tied to the account owner. This is a platform constraint, not a fixable gap.

---
CHANGELOG
v3 | 2026-05-26 | Added: Presentation tool rule (LLM-agnostic). Covers Gamma/Canva data integrity, workflow, and access constraints.
v2 | 2026-05-08 | Added: Prediction Lockout Rule.
v1 | Initial | Base preferences established.
